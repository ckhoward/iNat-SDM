...
def read_from_input_file(fname):
...
def main():
    ...
    if len(in_file) == 0:
        ...
        print("here")
        input_d = read_from_input_file(in_file)
        date1 = ''.join(input_d['date1'])
        print(date1)
        date2 = ''.join(input_d['date2'])
        calls_delta = input_d['call_rate']
        fname = input_d['out_file']
        s = ''.join(input_d['specific_species'])
        
        id_species = []
        
        t_ids = Taxa_Getter.get_taxa_id(s)
        
        for i in t_ids:
            temp = {}
            temp[i] = s
            id_species.append(temp)
